...
export default function EditQuizError({ reset }: { reset: () => void }) {
  return (
    <main className="min-h-screen bg-canvas px-6 py-12">
      <div className="mx-auto max-w-xl rounded-[12px] border border-line bg-surface p-8 shadow-soft">
        <h1 className="text-xl font-semibold tracking-tight text-ink">Не удалось загрузить редактор</h1>
        <p className="mt-2 text-sm text-muted">Проверьте доступ к квизу или повторите запрос.</p>
        <button
          type="button"
          onClick={reset}
